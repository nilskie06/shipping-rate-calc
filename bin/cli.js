...
// @ph-itdev/shipping-rate-calc CLI
// Usage: npx @ph-itdev/shipping-rate-calc playground
...
if (command === 'playground' || command === 'demo' || command === 'play') {
  ...
  Commands:
    npx @ph-itdev/shipping-rate-calc playground   Open interactive playground in browser
    npx @ph-itdev/shipping-rate-calc demo         Same as playground
    npx @ph-itdev/shipping-rate-calc help          Show this help
  `);
} else {
...
function startPlayground() {
  ...
  if (!fs.existsSync(examplePath)) {
    console.error('❌ Example file not found. Try: npm install @ph-itdev/shipping-rate-calc');
    process.exit(1);
  }
  ...
  server.listen(PORT, '127.0.0.1', () => {
    ...
    console.log(`
  📦 @ph-itdev/shipping-rate-calc — Playground running!
